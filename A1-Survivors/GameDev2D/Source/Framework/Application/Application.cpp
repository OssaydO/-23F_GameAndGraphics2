...
namespace GameDev2D
{
    ...
    void Application::OnUpdate(float delta)
    {
        if (m_IsRunning == true)
        {
            if (m_State == State::Splash)
            {
                ...
                {
                    m_State = State::StartDeathAnim;
                }
            }
            else if (m_State == State::StartDeathAnim)
            {
                m_FadeElapsed += delta;
    ...
    {
        if (m_IsRunning == true && (m_State == State::Game || m_State == State::StartDeathAnim || m_State == State::FadeIn))
        {
            if (m_IsSuspended == false)
            {
